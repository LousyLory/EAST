...
def sort_rectangle(poly):
	...
	if poly[p_lowest][0] - poly[p_lowest_right][0] == 0:
	    angle = np.pi/2.0
	else:
	    #####################################################################################################################
            #angle = np.arctan(-(poly[p_lowest][1] - poly[p_lowest_right][1])/(poly[p_lowest][0] - poly[p_lowest_right][0]))
	    angle = np.arctan2(-(poly[p_lowest][1] - poly[p_lowest_right][1])/(poly[p_lowest][0] - poly[p_lowest_right][0]))
	    #####################################################################################################################
        # assert angle > 0
        if angle <= 0:
            abcd=1#print(angle, poly[p_lowest], poly[p_lowest_right])
...
def generator(input_size=512, batch_size=32, data_path="train",
              background_ratio=3./8, trainOrVal = "train",
              random_scale=np.array([0.5, 1, 2.0, 3.0]),
              vis=False):
    with open('Data/cropped_annotations_'+trainOrVal+'.txt', 'r') as f:
    	annotation_file = f.readlines()
    index = []
    idx = 0
    for line in annotation_file:
	if len(line)>1 and line[:11] == 'cropped_img': 
	    if (trainOrVal=="train" and line[12:25] in training_list) or (trainOrVal=="val" and line[12:25] in validation_list): 
	        index.append(idx)
	idx += 1
    #print len(index)
    ...
    while True:
        ...
        for i in index:
            try:
		im_fn = 'detection/draw_1channel/'
                #im_fn = 'Data/cropped_img_' + data_path + '/'
		for fml in range(12,len(annotation_file[i])-1):
		...
		if im is None:
		    ...
		    for idx in range(i+2,i+2+int(annotation_file[i+1])):
		        ...
		        text_tags.append(False)
                    text_polys, text_tags = np.array(text_polys), np.array(text_tags, dtype=np.bool)
 		    ...
 		    labels.append(text_polys)
                if len(images) == batch_size:
                    if trainOrVal == "train":
                    	yield images, image_fns, score_maps, geo_maps, training_masks, labels
		    elif trainOrVal == "val":
			yield images, labels
                    images = []
                    image_fns = []
                ...
                continue
    '''
    with open('Data/cropped_annotations_train.txt', 'r') 
		for idx in range(i+2,i+2+int(annotation_file[i+1])):
		    text_polys.append([[int(x),int(y-h)],[int(x+w),int(y-h)],[int(x+w),int(y)],[int(x),int(y
                ...
                #txt_fn = im_fn.replace(os.path.basename(im_fn).split('.')[1], '
                #    print('text file {} does not exists'.format(txt_fn[0:11]+'gt_'+txt_fn[1
                #text_polys, text_tags = load_annoataion(txt_fn)
                #text_polys, text_tags = check_and_validate_polys(text_polys, text_tags,
                rd_scale = np.random.choice(random_scale)
                im = cv2.resize(im, dsize=None, fx=rd_scale, fy=rd_sca
                if np.random.rand() < background_ra
                    im, text_polys, text_tags = crop_area(im, text_polys, text_tags, 
    '''

def get_batch(num_workers, q_size=10, **kwargs):
    try:
        enqueuer = GeneratorEnqueuer(generator(**kwargs), use_multiprocessing=True)
        enqueuer.start(max_queue_size=q_size, workers=num_workers)
