...
def main(argv=None):
    ...
    learning_rate = tf.train.exponential_decay(FLAGS.learning_rate, global_step, decay_steps=10000, decay_rate=0.01, staircase=True)
    # add summary
    ...
    train_data_indices = []
    list_of_img_pos = []
    with open('Data/cropped_annotations_new/cropped_annotations' + my_char_l + '.txt', 'r') as f:
        ...
        for step in range(24*epoche_size):
            ...
            while len(data[0]) < 32:
                ...
                if prob > 0.49:
                    ...
                    im_fn = "Data/cropped_img/"+annotation_file[i][14:].split(".tiff",1)[0]+".tiff"
                    im = cv2.imread(im_fn)
                    if im is not None:
                        r, c, _ = im.shape
		        text_polys = []
