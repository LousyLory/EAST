...
def mean_image_subtraction(images, eval, means=[123.68, 116.78, 103.94]):
    '''
    image normalization
    :param images:
    :param means:
...
def model(images, weight_decay=1e-5, is_training=True, eval=False):
    ...
    with tf.variable_scope('feature_fusion', values=[end_points.values]):
	    ...
	    angle_map = 2 * slim.conv2d(g[3], 1, 1, activation_fn=tf.atan, normalizer_fn=None) # angle between (-pi, pi)
	    ############################################################################################################################
...
def loss(y_true_cls, y_pred_cls,
    ...
    return tf.reduce_mean(L_g * y_true_cls * training_mask) + 1.5*classification_loss
