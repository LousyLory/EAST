...
import numpy as np
import lanms
import model_4channels as model
import logging
...
def evaluateModel(model_ver, model_path, angles, imname_list, im_dir, annot_dir, results_dir, nms_thres, score_map_thresh, box_thresh, inspect_mode=False, calc_p_r=False):
    ...
    with tf.get_default_graph().as_default():
        input_images = tf.placeholder(tf.float32, shape=[None, None, None, 4], name='input_images')
		...
		for rot in angles:
			...
			score, geometry = sess.run([f_score, f_geometry], feed_dict={input_images: images})
                        for j in range(num_cols):
			    boxes = detect(score_map=score[j], geo_map=geometry[j], score_map_thresh=score_map_thresh, box_thresh=box_thresh_loc, nms_thres=nms_thres)
			    if boxes is not None:
                                boxes = boxes[:, :8].reshape((-1, 4, 2))
                                for box in boxes:
...
def main(argv):
	...
	test_list = ["D0117-5755035", "D0117-5755036", "D5005-5028052", "D5005-5028054", \
        ...
        model_ver = "_4_channel_3"
        model_path = "tmp/east_icdar2015_resnet_v1" + model_ver
	#print sys.argv
	imname_list = [sys.argv[1]]#["D0117-5755036"]#["D0117-5755035"]#, "D0089-5235001"]  #training_list + validation_list + test_list 
        im_dir = "Data/maps/"
        annot_dir = "Data/maps/"
        results_dir = "Results/"
	lanms_thresh = 0.1
        score_map_thresh = 0.86#0.15
        box_thresh = [0.01, 0.2]
        inspect_mode = False
        calc_p_r = True
